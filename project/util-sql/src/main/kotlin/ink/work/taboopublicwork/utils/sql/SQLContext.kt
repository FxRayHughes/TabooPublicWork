...
interface SQLContext {
    ...
    fun initDataSource(module: IModule) {
        // 首先判断子文件是否写入了数据源
        var host = SQLUtil.config.getString("database.host", "localhost")
        var port = SQLUtil.config.getString("database.port", "3306")
        var user = SQLUtil.config.getString("database.user", "root")
        var password = SQLUtil.config.getString("database.password", "root")
        var database = SQLUtil.config.getString("database.database", "minecraft")
        // 判断私有源
